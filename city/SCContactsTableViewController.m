...
- (void)viewDidLoad
{
   ...
   ABAddressBookRequestAccessWithCompletion(addressBook, ^(bool granted, CFErrorRef error) {
      ...
      if (granted) {
         ...
         UIAlertView* av = [[UIAlertView alloc] initWithTitle:@"Whoops!" message:@"You must grant us access to your contacts in order to invite friends." delegate:self cancelButtonTitle:@"Okay." otherButtonTitles:nil];
         [av show];
      }
   });
...
- (void) setCancelButton
{
   ...
   [self.navigationItem setLeftBarButtonItem:[[UIBarButtonItem alloc] initWithTitle:self.cancelButtonText style:UIBarButtonItemStylePlain target:self action:@selector(cancel)]];
}

- (void) setTitleText
{
   NSDictionary *attributes = [NSDictionary dictionaryWithObjectsAndKeys:[[BTSession thisSession] helveticaFont:18],NSFontAttributeName, [UIColor blackColor], NSForegroundColorAttributeName, nil];
   ...
   for ( int i = 0; i < contactCount; i++ )
   {
      ...
      if (ABMultiValueGetCount(multi)){
         ...
         for ( int i = 0; i < ABMultiValueGetCount(multi); i++ ) {
            [phone setObject:@"phone" forKey:(__bridge NSString*)(ABMultiValueCopyValueAtIndex(multi,i))];
         }
...
//           }
           ...
           NSString *recordOne;
           if (([[[array objectAtIndex:i] valueForKey:@"firstName"] length] > 0)) {
              recordOne = [[[array objectAtIndex:i] valueForKey:@"firstName"] lowercaseString];
           } else {
              recordOne = [[[array objectAtIndex:i] valueForKey:@"name"] lowercaseString];
           }
           
           NSString *recordTwo;
           if (([[contact valueForKey:@"firstName"] length] > 0)) {
              recordTwo = [[contact valueForKey:@"firstName"] lowercaseString];
           } else {
              recordTwo = [[contact valueForKey:@"name"] lowercaseString];
           }
...
{
   if (first && [[self contactsToWorkWith] count] == 0) {
      first = NO;
      [self showHUD];
   } else {
      [self hideHUD];
...
//      [self.delegateView contactsSelectorReturnsWithArrayOfContacts:self.selectedContacts];
   }];
}
